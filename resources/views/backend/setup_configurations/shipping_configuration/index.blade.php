        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('shipping_configuration.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="radio mar-btm">
                        <input id="area-shipping" class="magic-radio" type="radio" name="shipping_type" value="weight_wise_shipping" <?php if(get_setting('shipping_type') == 'weight_wise_shipping') echo "checked";?>>
                        <label for="area-shipping">{{translate('Weight Wise Flat Shipping Cost')}}</label>
                    </div>
...
@if (get_setting("shipping_type") == "weight_wise_shipping")
<div class="row">
    <div class="col-lg-6">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{translate('Weight Wise Shipping')}}</h5>
            </div>
            <form action="{{ route('shipping_configuration.update') }}" method="POST" enctype="multipart/form-data">
              <div class="card-body">
                  @csrf
                  <input type="hidden" name="type[]" value="cost_for_2kg">
                  <input type="hidden" name="type[]" value="cost_per_kg_after_2kg">
                  <div class="form-group">
                        <label for="">{{ translate('Cost For 2KG') }}</label>
                        <input class="form-control" type="text" name="cost_for_2kg" value="{{ get_setting('cost_for_2kg') }}">
                  </div>
                  <div class="form-group">
                    <label for="">{{ translate('Cost Per KG After 2KG') }}</label>
                    <input class="form-control" type="text" name="cost_per_kg_after_2kg" value="{{ get_setting('cost_per_kg_after_2kg') }}">
                  </div>
                  <div class="form-group mb-0 text-right">
                      <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                  </div>
