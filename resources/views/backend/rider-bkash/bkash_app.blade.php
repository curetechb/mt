...
<body>
    ...
    <script type="text/javascript">
        ...
        paymentRequest: {
            amount: '{{ $rider->total_collection }}', //max two decimal points allowed
            intent: 'sale'
        },
        ...
        $.ajax({
          url: '{{ route('rider.bkash.checkout') }}',
          type: 'POST',
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (result && result.paymentID != null) {
                window.location.href = "{{ route('rider.bkash.success') }}?payment_details="+data; //Merchant’s success page
            } else {
                window.location.href = "{{ route('rider.bkash.fail') }}?payment_details="+data; //Merchant’s success page
