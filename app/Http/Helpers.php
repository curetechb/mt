...
if (! function_exists('discount_in_percentage')) {
    function discount_in_percentage($product)
    {
        try {
            ...
            $dp = $base > 0 ? ($discount * 100) / $base : $discount;
            return round($dp);
        } catch (Exception $e) {
...
if (!function_exists('uploaded_asset')) {
    function uploaded_asset($id)
    {
        ...
        // }
        // return null;
        $asset = \App\Models\Upload::find($id, ['file_name', 'external_link']);
        if ($asset != null) {
            return $asset->external_link == null ? my_asset($asset->file_name) : $asset->external_link;
        }
        return null;
...
// Addon Activation Check
if (!function_exists('get_delivery_times')) {
    function get_delivery_times($current_date)
    {
        $times = [
            [
                "start_time" => "9:00AM",
        ...
        foreach($times as $i => $t){
            ...
            // only consider time between 12:00pm to 9:00pm
            if(time() > strtotime("9:00AM") && time() < strtotime("6:00PM")){

                // if day is current day and
                if($current_date == 1 && strtotime($t['start_time']) < time()) continue;
                // if($current_date == 1 && time() > strtotime($t['start_time']) && time() < strtotime($t['end_time'])) continue;
...
if(!function_exists("get_delivery_dates")){

    function get_delivery_dates($limit = 4){

        $delivery_dates = [];

        foreach (range(0,$limit) as $i => $day) {

            if($i == 0){
                if(time() >= strtotime("12:00AM") && time() <= strtotime("6:00PM")){
                    array_push($delivery_dates, [
                        "value" => $i+1,
                        "date" => date('Y-m-d', strtotime("+$i day")),
                        "text" => "Today - ".date('d M', strtotime("+$i day")),
                        "disabled" => false,
                    ]);
                }
            }else if($i == 1){
                array_push($delivery_dates, [
                    "value" => $i+1,
                    "date" => date('Y-m-d', strtotime("+$i day")),
                    "text" => "Tomorrow - ".date('d M', strtotime("+$i day")),
                    "disabled" => false,
                ]);
            }else{
                array_push($delivery_dates, [
                    "value" => $i+1,
                    "date" => date('Y-m-d', strtotime("+$i day")),
                    "text" => date('l', strtotime("+$day day"))." - ".date('d M', strtotime("+$i day")),
                    "disabled" => false,
                ]);
            }
