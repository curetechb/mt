...
class HomeController extends Controller
{
    ...
    {
        // $categories = Category::where("parent_id", 0)->orderBy("order_level", "DESC")->get(['slug', 'name', 'icon']);

        $categories = Category::orderBy("order_level", "DESC")->get(['slug', 'name', 'icon']);
    ...
    public function cart_login(Request $request)
    {
        ...
        if($user != null){
            if(Hash::check($request->password, $user->password)){
                ...
                Session::flash("error",translate('Invalid email or password!'));
            }
        }
        else{
            Session::flash("error",translate('Invalid email or password!'));
...
//        $categories = Category::where('level', 0)->orderBy('name', 'asc')->get();
    ...
    public function show_product_upload_form(Request $request)
    {
        ...
        if(addon_is_activated('seller_subscription')){
            if($seller->seller_package && $seller->seller_package->product_upload_limit > $seller->user->products()->count()){
                ...
                Session::flash("error",translate('Upload limit has been reached. Please upgrade your package.'));
                return back();
            }
        }
        $categories = Category::where('parent_id', 0)
    ...
    public function top_10_settings(Request $request)
    {
        ...
        Session::flash("success",translate('Top 10 categories and brands have been updated successfully'));
    ...
    public function update_email(Request $request)
    {
        ...
        if(isUnique($email)) {
            ...
            Session::flash("success",translate('A verification mail has been sent to the mail you provided us with.'));
            return back();
        }

        Session::flash("error",translate('Email already exists!'));
        return back();
    }
    ...
    public function email_change_callback(Request $request){
        if($request->has('new_email_verificiation_code') && $request->has('email')) {
            ...
            if($user != null) {
                ...
                Session::flash("success",translate('Email Changed successfully'));
                return redirect()->route('dashboard');
            }
        }

        Session::flash("error",translate('Email was not verified. Please resend your mail!'));
        return redirect()->route('dashboard');
    ...
    public function reset_password_with_code(Request $request){
        if (($user = User::where('email', $request->email)->where('verification_code', $request->code)->first()) != null) {
            if($request->password == $request->password_confirmation){
                ...
                Session::flash("error","Password and confirm password didn't match");
    ...
    public function rewardHistories(){
        ...
        return view("frontend.user.reward_histories", compact("accrued", "redeem"));
    }
}
