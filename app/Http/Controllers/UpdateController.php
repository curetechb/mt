...
class UpdateController extends Controller
{
    public function step0(Request $request) {
        if (env('DEMO_MODE') == 'On') {
            Session::flash("error",translate('This action is disabled in demo mode'));
            return back();
        }
        if ($request->has('update_zip')) {
            if (class_exists('ZipArchive')) {
                ...
                if ($res === true) {
                    ...
                    Session::flash("error",translate('Could not open the updates zip file.'));
                    return back();
                }
                ...
                Session::flash("error",translate('Please enable ZipArchive extension.'));
            }
        }
        else {
