...
class AizUploadController extends Controller
{
    ...
    public function upload(Request $request){
        ...
        if($request->hasFile('aiz_file')){
            ...
            // $reqarr = explode('.', $request->file('aiz_file')->getClientOriginalName());
            // array_pop($reqarr);
            // $original_name = implode("", $reqarr);
            // $up = Upload::where("file_original_name", $original_name)->first();

            // if($up){
            //     return response()->json("Already uploaded", 422);
            // }
