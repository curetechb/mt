...
class _RiderController extends Controller
{
    ...
    public function riderRegistration(StoreRiderRequest $request){
        ...
        try{
            ...
            if($request->avatar){
                // $path = Storage::disk('s3')->put('riders/avatar', $request->avatar, ['visibility' => 'public']);
            ...
            // $request->merge(['aiz_file' => $request->avatar_original, 'user_id' => $user->id]);
            ...
            if($request->nid_image){
                // $path = Storage::disk('s3')->put('riders/nid', $request->nid_image, ['visibility' => 'public']);
                // $path = Storage::disk('s3')->url($path);
                $nid_upload_id = $this->uploadRiderFile($request->nid_image);
                $delivery_boy->nid_image      = $nid_upload_id;
            }
            if($request->nid_image_backpart){
                // $path = Storage::disk('s3')->put('riders/nid', $request->nid_image_backpart, ['visibility' => 'public']);
                // $path = Storage::disk('s3')->url($path);
                $nid_image_backpart_id = $this->uploadRiderFile($request->nid_image_backpart);
                $delivery_boy->nid_image_backpart      = $nid_image_backpart_id;
            }
            $delivery_boy->dob      = date('Y-m-d', strtotime($request->dob));
            ...
            if($request->license_image){
                // $path = Storage::disk('s3')->put('riders/license', $request->license_image, ['visibility' => 'public']);
                // $path = Storage::disk('s3')->url($path);
                $license_upload_id = $this->uploadRiderFile($request->license_image);
                $delivery_boy->license_image = $license_upload_id;
            }
            if($request->license_image_backpart){
                // $path = Storage::disk('s3')->put('riders/license', $request->license_image_backpart, ['visibility' => 'public']);
                // $path = Storage::disk('s3')->url($path);
                $license_back_upload_id = $this->uploadRiderFile($request->license_image_backpart);
                $delivery_boy->license_image_backpart = $license_back_upload_id;
            }
            ...
            return response()->json([
                "success" => true,
                "message" => "Thank you for Registering. You will be notified by SMS"
            ]);
