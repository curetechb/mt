...
class LanguageController extends Controller
{
    public function changeLanguage(Request $request)
    {
    	...
    	//Session::flash("success",translate('Language changed to ').$language->name)->success();
    }
    ...
    public function store(Request $request)
    {
        if(Language::where('code',$request->code)->first()){
            Session::flash("error",translate('This code is already used for another language'));
            return back();
        }
        ...
        Session::flash("success",translate('Language has been inserted successfully'));
    ...
    public function update(Request $request, $id)
    {
        if(Language::where('code', $request->code)->where('id', '!=', $id)->first()){
            Session::flash("success",translate('This code is already used for another language'));
            return back();
        }
        $language = Language::findOrFail($id);
        if (env('DEFAULT_LANGUAGE') == $language->code && env('DEFAULT_LANGUAGE') != $request->code) {
            Session::flash("error",translate('Default language code can not be edited'));
            return back();
        } elseif ($language->code == 'en' && $request->code != 'en') {
            Session::flash("error",translate('English language code can not be edited'));
            return back();
        }
        ...
        Session::flash("success",translate('Language has been updated successfully'));
        return redirect()->route('languages.index');
    }
    ...
    public function key_value_store(Request $request)
    {
        ...
        Session::flash("success",translate('Translations updated for ').$language->name);
        return back();
    }
    ...
    public function update_status(Request $request)
    {
        ...
        if($language->code == env('DEFAULT_LANGUAGE') && $request->status == 0) {
            Session::flash("success",translate('Default language can not be inactive'));
            return 1;
        }
        $language->status = $request->status;
        if($language->save()){
            Session::flash("success",translate('Status updated successfully'));
            return 1;
        }
        return 0;
    }
    ...
    public function update_rtl_status(Request $request)
    {
        ...
        if($language->save()){
            Session::flash("success",translate('RTL status updated successfully'));
            return 1;
        }
        return 0;
    }
    ...
    public function destroy($id)
    {
        ...
        if (env('DEFAULT_LANGUAGE') == $language->code) {
            Session::flash("success",translate('Default language can not be deleted'));
        } elseif($language->code == 'en') {
            Session::flash("success",translate('English language can not be deleted'));
        }
        else {
            ...
            Language::destroy($id);
            Session::flash("success",translate('Language has been deleted successfully'));
        }
        return redirect()->route('languages.index');
    }
    ...
    public function importEnglishFile(Request $request){
        ...
        Session::flash("success",translate('Translation keys has been imported successfully. Go to App Translation for more..'));
        return back();
    }
    ...
    public function storeAppTranlsation(Request $request){
        ...
        Session::flash("success",translate('App Translations updated for ').$language->name);
        return back();
    }
