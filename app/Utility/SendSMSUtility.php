...
class SendSMSUtility
{
    ...
    {
        if(OtpConfiguration::where('type', 'bulksmsbd')->first()->value == 1){

            $number = trim($to, '+');

            $url = "http://66.45.237.70/api.php";
            $data= array(
                'username' => env("BULK_SMS_BD_USERNAME"),
                'password' => env("BULK_SMS_BD_PASSWORD"),
                'number' => "$number",
                'message' => "$text",
                'senderid' => "Muslim Town"
            );
        ...
        }else if (OtpConfiguration::where('type', 'nexmo')->first()->value == 1) {
